...
import { BookOpen, Clock, ExternalLink, Heart, User } from 'lucide-react';
...
export const BlogCard: React.FC<BlogCardProps> = ({ result, index }) => {
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-lg leading-tight hover:text-green-600 transition-colors line-clamp-2">
                  {result.title}
                </h3>
          ...
          {result.aiSummary && (
            <div className="p-3 bg-green-50 dark:bg-green-950/20 rounded-lg border border-green-200 dark:border-green-800">
              <p className="text-sm text-green-700 dark:text-green-300">
                <span className="font-medium">Summary:</span> {result.aiSummary}
              </p>
            </div>
          )}
          ...
          <div className="flex items-center space-x-4 text-sm text-muted-foreground">
            {result.metadata.readTime && (
              <div className="flex items-center space-x-1">
                ...
                <span>{result.metadata.readTime}</span>
              </div>
            )}
          </div>
